...
def final_out_put(pdf_path):
    ...
    try:
        ...
        if len(list_number) > 0:
            ...
            #output_copy_path=shutil.copy(output_json_path, "final_output")
...
filename="RTA0924.pdf"
file_name="output\RTA0924_output.json"
file_path=final_out_put(filename)
print(file_path)
page_number=str(store_data(file_name)[0])
number=int(page_number[-1])-1

output_dir = os.path.join(os.getcwd(), "output_folder")
output_file = os.path.join(output_dir, f"{page_number}_page.pdf")
...
if total_pages >= 2:
    # Initialize PdfWriter to store the extracted page
    writer = PdfWriter()
    # Extract the second page (index 1 for page 2)
    page_2 = reader.pages[number]  # The second page is at index 1
    ...
    print(f"Page 2 saved to {output_file}")
else:
